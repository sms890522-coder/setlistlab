...
- 기본 팀 월간 녹음실 생성: 3개
- 기본 곡/세션당 활성 트랙: 12개
- 같은 사용자와 같은 파트의 재녹음은 최근 2개만 활성 보관합니다.
- 실험실을 켠 사용자는 테스트 목적으로 quota 제한을 우회하지만, 팀 승인 멤버 권한과 R2 presigned URL 보안 검사는 그대로 적용됩니다.
- 허용 MIME:
  - `audio/webm`
...
- `replaced` 상태인데 `object_key`가 아직 남아 있는 트랙

기본 보관 기간은 60일입니다. MVP에서는 사용자가 모르는 사이 active 녹음이 사라지지 않도록 자동 삭제하지 않고 안내와 TODO로만 남깁니다.
