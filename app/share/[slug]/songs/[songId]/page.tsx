...
export default function LegacySharedSongPage() {
  const params = useParams<{ slug: string; songId: string }>();
  ...
  useEffect(() => {
    async function loadSetlist() {
      if (!isSupabaseConfigured()) {
        setError("공유 기능이 아직 준비되지 않아 곡을 불러올 수 없습니다.");
        setLoaded(true);
        return;
      }
  ...
  return (
    <SharedSongPractice
      ...
      songHref={(nextSongId) => `/share/${params.slug}/songs/${nextSongId}`}
    />
  );
}
