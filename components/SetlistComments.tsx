...
import {
  createSetlistComment,
  deleteSetlistComment,
  getSetlistComments,
  subscribeSetlistComments,
  updateSetlistComment,
  type SetlistComment,
} from "@/lib/db/setlistComments";
import { formatMemberNameWithEmoji } from "@/lib/roleEmoji";
import { useEffect, useState } from "react";

type SetlistCommentsProps = {
  setlistId: string;
  canComment: boolean;
...
export function SetlistComments({
  setlistId,
  canComment,
  canManageComments = false,
  scopeLabel = "콘티 댓글",
}: SetlistCommentsProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([getCurrentUser(), getSetlistComments(setlistId)])
  ...
  useEffect(() => {
    return subscribeSetlistComments(
      setlistId,
      (comment, event) => {
        if (event === "DELETE") {
  ...
  async function handleCreateComment() {
    ...
    try {
      const comment = await createSetlistComment({ setlistId, content });
      setComments((current) => upsertComment(current, comment));
      setContent("");
    } catch (createError) {
      setError(createError instanceof Error ? createError.message : "댓글을 등록하지 못했습니다. 다시 시도해 주세요.");
    } finally {
      setSubmitting(false);
    }
  }

  function startEditing(comment: SetlistComment) {
  ...
  async function handleUpdateComment(commentId: string) {
    ...
    try {
      const comment = await updateSetlistComment(commentId, editingContent);
      setComments((current) => upsertComment(current, comment));
      setEditingId(null);
      setEditingContent("");
    } catch (updateError) {
  ...
  async function handleDeleteComment(comment: SetlistComment) {
    ...
    setError("");
    try {
      const deletedComment = await deleteSetlistComment(comment.id);
      ...
      <div className="border-b border-slate-100 bg-gradient-to-r from-blue-50 via-white to-violet-50 px-5 py-5 sm:px-7">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full bg-blue-600 px-3 py-1.5 text-xs font-black text-white">{scopeLabel}</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-5 p-4 sm:p-6 lg:p-7">
        {canComment ? (
          <div className="rounded-2xl border border-blue-100 bg-blue-50/60 p-4 sm:p-5">
            <label className="text-sm font-black text-slate-950" htmlFor={`setlist-comment-content-${setlistId}`}>
              새 댓글 작성
            </label>
            <textarea
              id={`setlist-comment-content-${setlistId}`}
              value={content}
              onChange={(event) => setContent(event.target.value)}
              maxLength={1000}
              className="field-input mt-3 min-h-32 resize-y bg-white text-[15px] leading-7 shadow-sm sm:min-h-36"
              placeholder="예: 확인했습니다. 2번 곡 키가 G 맞나요?"
            />
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-5 py-8 text-center">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">콘티에 대한 확인 메시지나 질문을 첫 댓글로 남겨보세요.</p>
            </div>
          ) : (
            comments.map((comment) => {
              ...
              const authorPosition = comment.authorMembership?.position || comment.author?.role || "팀원";
                        ...
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <p className="font-black text-slate-950">
                              {comment.authorMembership ? formatMemberNameWithEmoji(authorPosition, authorName) : authorName}
                            </p>
                            {comment.authorMembership ? <TeamRoleBadge role={role} /> : null}
...
function upsertComment(comments: SetlistComment[], comment: SetlistComment) {
