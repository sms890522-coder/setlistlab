  ...
  useEffect(() => {
    ...
    async function loadCharacter() {
      ...
      try {
        ...
        if (cancelled) return;
        setCharacter(normalizeCharacterConfig(response));
    ...
    <section className="card overflow-hidden">
      ...
      <div className="grid gap-5 p-5 sm:grid-cols-[180px_1fr] sm:p-6">
        <div className="flex justify-center sm:justify-start">
          <CharacterImage src={character.imageUrl} alt={`${getCharacterSummary(character)} 캐릭터`} size="md" />
        </div>
        <div className="flex flex-col justify-center">
          <p className="text-sm font-bold text-slate-500">{hasCharacter ? "현재 선택" : "아직 선택한 캐릭터가 없습니다."}</p>
          <p className="mt-1 text-xl font-black text-slate-950">{hasCharacter ? getCharacterSummary(character) : "기본 캐릭터"}</p>
          <p className="mt-2 text-sm leading-6 text-slate-600">
            {hasCharacter
              ? "캐릭터 이미지는 성별과 악기/역할 조합으로 정해집니다. 나중에 무대배치도에서 팀원 캐릭터로 사용할 수 있습니다."
...
function CharacterBuilderModal({
  ...
  async function handleSave() {
    ...
    try {
      const response = await fetchCharacter("POST", { gender, instrument });
      onSaved(normalizeCharacterConfig(response));
    } catch (saveError) {
      setStatus("error");
      ...
      <div className="max-h-[92vh] w-full overflow-y-auto rounded-t-[2rem] bg-white shadow-2xl sm:max-w-4xl sm:rounded-[2rem]">
        ...
        <div className="grid gap-6 p-5 sm:grid-cols-[280px_1fr] sm:p-6">
          <aside className="flex flex-col items-center gap-4 rounded-3xl border border-slate-100 bg-slate-50 p-5">
            ...
            <CharacterImage src={previewCharacter.imageUrl} alt={`${getCharacterSummary(previewCharacter)} 캐릭터 미리보기`} size="lg" />
            <div className="w-full rounded-2xl bg-white p-4 text-center">
              ...
              <p className="mt-1 text-xs leading-5 text-slate-500">현재 조합의 이미지 1개만 불러옵니다.</p>
            </div>
          </aside>
...
type CharacterResponse = CharacterConfig & {
  hasCharacter?: boolean;
  updatedAt?: string | null;
};

async function fetchCharacter(method: "GET", body?: never): Promise<CharacterResponse>;
async function fetchCharacter(method: "POST", body: { gender: CharacterGender; instrument: CharacterInstrument }): Promise<CharacterResponse>;
async function fetchCharacter(method: "GET" | "POST", body?: { gender: CharacterGender; instrument: CharacterInstrument }) {
