          ...
          <div className="max-h-[calc(100dvh-2.5rem)] overflow-y-auto p-5 sm:max-h-[calc(100dvh-4rem)] sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 flex-1">
                ...
                <h2 className="mt-3 break-words text-2xl font-black tracking-tight text-slate-950">{announcement.title}</h2>
                <p className="mt-2 text-sm font-semibold leading-6 text-slate-600">{announcement.summary}</p>
              </div>
              <button type="button" onClick={onClose} className="shrink-0 rounded-xl bg-slate-100 px-3 py-2 text-sm font-black text-slate-500">
                닫기
              </button>
            </div>
